...
<body>
	...
	<hr>
		<div id=#jb-container>
			<span class=title style="float:left;">Part All</span>
			
			<c:set var="counter" value="${counter+1 }"></c:set>
			<c:forEach var="vo" items="${jpttot }" varStatus="status">
			...
			<div class="jb-content clear">
				...
				<span class="title clear"><input type="checkbox" name="forvite${counter}" value= "${vo.qidx }"  <c:if test = "${favoriteSet.contains(vo.qidx) }"> checked</c:if> ></span><br>
				<div class="radio-group">
					<label class=radio>
						<input type="radio" name="group${counter}" value="1">1. ${vo.choice[0]}
						<span></span>
					</label><br>
					<label class=radio>
						<input type="radio" name="group${counter}" value="2">2 .${vo.choice[1]}
						<span></span>
					</label><br>
					<label class=radio>
						<input type="radio" name="group${counter}" value="3">3 .${vo.choice[2]}
						<span></span>
					</label><br>
					<label class=radio>
						<input type="radio" name="group${counter}" value="4">4 .${vo.choice[3]}
						<span></span>
					</label>
					<c:set var="counter" value="${counter+1 }"></c:set>
					</form>
			...
			<div class="jb-content clear">
				...
				<span class="title clear"><input type="checkbox" name="forvite${counter}" value= "${vo.qidx }" > </span><br>
				<div class="radio-group">
					<label class=radio>
	...
	<div id=RightBottomLayerFixed>
		<input class="btns" type="button" value="제출" id="submit" >
	<input class="btnc" type="button" value="취소" onclick="window.history.back()" >
	</div>
	...
	$("#submit").click(function() {
		...
		$.ajax({
	        ...
	        success: function(data){
	        	post_to_url("question?cmd=testEnd", {arg: $("form").serialize()});
	        }
	    });
